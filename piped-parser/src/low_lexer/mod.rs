...
fn next(input: impl AsRef<str>) -> LowToken {
    ...
    let kind = match cursor.consume().unwrap() {
        ...
        '~' => LowTokenKind::Tilde,
        '\'' => LowTokenKind::Literal {
            kind: LowTokenLiteralKind::SingleQuotedString {
                terminated: consume_single_quoted_literal_string(&mut cursor),
            },
        },
        '"' => LowTokenKind::Literal {
            kind: LowTokenLiteralKind::DoubleQuotedString {
                terminated: consume_double_quoted_literal_string(&mut cursor),
            },
        },
...
fn consume_single_quoted_literal_string(cursor: &mut Cursor) -> bool {
...
fn consume_double_quoted_literal_string(cursor: &mut Cursor) -> bool {
    while let Some(char) = cursor.consume() {
        match char {
            '"' => return true,
            '\\' => {
